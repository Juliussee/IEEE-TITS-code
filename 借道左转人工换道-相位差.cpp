...
int velocitySum(CARL* car_left, CARR* car_right) {
	...
	for (i = 0; i <= veh_sum_right; i++) {
		if (LA <= car_right[i].x && car_right[i].x < LA+LB) { //找出LB区域右车道满足换道车辆，使用参数isinsert==1来表示
			...
			if(light_time<65-light_t1||light_time>=110-light_t2) {//预信号灯控制
...
int main() {
	...
	FILE* fp;
	if ((fp = fopen("借道左转 流量变化图 t1=25 t2=15.txt", "a+")) == NULL) {//这里修改5  6  10  20
	...
	for(Qin=0.1; Qin<1; Qin+=0.1) {
		...
		fprintf(fp, "%d ", LB_change_num); //表示换道车辆数
