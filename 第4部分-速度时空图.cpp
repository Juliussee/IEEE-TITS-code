...
int main() {
	...
	FILE* fp1;
	if ((fp1 = fopen("左车道 Qin=0.2 r_pro=0.8.txt", "a+")) == NULL) {//这里修改5  6  10  20
		printf("cannot open this file.\n");
		exit(0);
	}

	FILE* fp2;
	if ((fp2 = fopen("右车道 Qin=0.2 r_pro=0.8.txt", "a+")) == NULL) {//这里修改5  6  10  20
		printf("cannot open this file.\n");
		exit(0);
	}
	...
	for (time_step = 0; time_step < total_timestep; time_step++) {
		...
		//时空图
		if (time_step >= 100000) {
//			printf("当前时步为：%d %d %d\n",time_step,veh_sum_left,veh_sum_right);

			for (j = 0; j < veh_sum_left; j++) {
				if(car_left[j].x<road_long_right&&car_left[j].x>=LA) {
					fprintf(fp1, "%d, ", (int)car_left[j].x);
					fprintf(fp1, "%d, ", time_step );
					fprintf(fp1, "%d\n", (int)car_left[j].v);
				}
			}
			for (j = 0; j < veh_sum_right; j++) {
				if(car_right[j].x<road_long_right&&car_right[j].x>=LA) {
					fprintf(fp2, "%d, ", (int)car_right[j].x);
					fprintf(fp2, "%d, ", time_step);
					fprintf(fp2, "%d\n", (int)car_right[j].v);
				}
			}
