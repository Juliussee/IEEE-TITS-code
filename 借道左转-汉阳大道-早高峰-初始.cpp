...
int velocitySum(CARL* car_left, CARR* car_right) {
	...
	/*step2*/
	/*判断车道需要换道的车辆*/
	light_time=time_step%(t1+t2+t3+t4);
	for (i = 0; i <= veh_sum_right; i++) {
		if (LA <= car_right[i].x && car_right[i].x < LA+LB) { //找出LB区域右车道满足换道车辆，使用参数isinsert==1来表示
			...
			if(light_time<t1||light_time>=t1+t2+t3) {//预信号灯控制
...
int main() {
	...
	FILE* fp;
	if ((fp = fopen("借道左转 延误对比 汉阳大道-早高峰 初始流量.txt", "a+")) == NULL) {//这里修改5  6  10  20
	...
	LB = 26;
	LC = 62;
	LA = road_long_right - LB-LC;

	test_cell=road_long_right-40;//表示监测点
