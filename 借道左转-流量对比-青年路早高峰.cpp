...
int main() {
	...
	FILE* fp;
	if ((fp = fopen("借道左转 流量对比 长丰大道晚高峰.txt", "a+")) == NULL) {//这里修改5  6  10  20
	...
	for(Qin=0.276; Qin<0.3; Qin+=1) {
		...
		fprintf(fp, "%f ", (double)coil_velosity_sum_right / coil_veh_sum_right); //表示速度

		fprintf(fp, "%f \n", (double)coil_velosity_sum_left / coil_veh_sum_left); //表示速度
	}
