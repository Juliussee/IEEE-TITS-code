...
#define total_timestep 11000000
#define count_timestep 10000000
...
int ramp_t1;//用于标定每周期左车道头车到达路口时间
int ramp_t2;


int isleave=0;//标定第一辆车出车
double green_v_sum=0;
int green_veh_sum=0;
...
double generate_normal_random(double mean, double stddev) {
	double u1 = (double)rand() / RAND_MAX; // [0,1) 区间的均匀随机数
	double u2 = (double)rand() / RAND_MAX; // [0,1) 区间的均匀随机数
...
int velocitySum(CARL* car_left, CARR* car_right) {
	...
	/*step2*/
	/*判断车道需要换道的车辆*/
	light_time=time_step%(T1+Tenter+T2+Tclear);
	//printf("%d ",T1+Tenter+T2+Tclear);
...
//		printf("当前时步：%d Tenter：%d Tclear：%d \n",time_step,Tenter,Tclear);

		ramp_t1=0;
		isleave=0;
		green_v_sum=0;//速度和归零
		green_veh_sum=0;//车辆数归零

	} else if(light_time>=T1+Tenter&&light_time<T1+Tenter+T2) {

		for(j=0; j<veh_sum_left; j++) {
			green_v_sum+=car_left[j].v;

			//printf("当前时步：%d 车总数：%d 速度：%f 速度和：%f\n",time_step,veh_sum_left,car_left[j].v,green_v_sum);
		}

		green_veh_sum+=veh_sum_left;

	} else if(light_time==T1+Tenter+T2) {

		double green_average_v=green_v_sum/green_veh_sum;

		//printf("%f %d %f \n",green_v_sum,green_veh_sum,green_average_v);

		double random_value = generate_normal_random(2.0, 1.0);

		if(veh_sum_left>0) {

			double last_gap=road_long_right-car_left[veh_sum_left-1].x-veh_length;

			double Twait=getDoubleMax2(0,ceil(veh_sum_left-(double)(last_gap/green_average_v)));

			ramp_t2=(int)(Twait+(double)(LC+LB-veh_length)/6)+(int)random_value;

//			printf("当前时步：%d ramp_t2：%d Twait:%f 左车道车辆数：%d 等待时间：%f 平均速度%f \n",time_step,ramp_t2,Twait,veh_sum_left,car_left[veh_sum_left-1].gap/car_left[veh_sum_left-1].v,green_average_v);

		} else {
			ramp_t2=(int)(0+(double)(LC+LB-veh_length)/6)+(int)random_value;
		}
	...
	for (i = 0; i <= veh_sum_right; i++) {
		if (LA <= car_right[i].x && car_right[i].x < LA+LB) { //找出LB区域右车道满足换道车辆，使用参数isinsert==1来表示
			...
			if(light_time<T1||light_time>=T1+Tenter+T2) {//预信号灯控制
	...
	//此处更新左车道头车
	if(light_time>T1&&light_time<T1+Tenter&&isleave==0) {
		ramp_t1++;
	}

	if(veh_sum_left==(int)((LB+LC)/8.5)) {
	...
	//printf("当前时步：%d ramp_t1值：%d 左车道总数：%d light_time:%d Tenter:%d\n",time_step,ramp_t1,veh_sum_left,light_time,Tenter);
...
void car_out(CARL* car_left,CARR* car_right) {
	...
	for (i = 0; i < veh_sum_left; i++) {
		if (car_left[i].x > road_long_right || car_left[i].x < 0) {
			printf("左车道道路格点：%d,%d\n", car_left[i].x, car_left[i].v);
		}
	}
}
...
int main() {
	...
	LB=25;
	LC=75;
	LA = road_long_right - LB-LC;//记住，LA计算必须要在LB赋值之后！
	...
	ramp_t1=Tenter;//用于标定每周期左车道头车到达路口时间
	ramp_t2=Tclear;
	...
	int CLdensity[11000]= {0};
	int RLdensity[11000]= {0};


	FILE* fp1;
	if ((fp1 = fopen("青年路 道路密度 Qin=0.3 r_pro=0.txt", "a+")) == NULL) {//这里修改5  6  10  20
	...
	for (time_step = 0; time_step < total_timestep; time_step++) {
		...
		if (time_step >= total_timestep - count_timestep) {
			...
			for (j = 0; j < veh_sum_left; j++) {
				...
				if (car_left[j].x>= LA-200) {

					int intPosi=(int)car_left[j].x;

					CLdensity[intPosi]+=1;
					CLdensity[intPosi+1]+=1;
					CLdensity[intPosi+2]+=1;
					CLdensity[intPosi+3]+=1;
					CLdensity[intPosi+4]+=1;
					CLdensity[intPosi+5]+=1;
					CLdensity[intPosi+6]+=1;
					CLdensity[intPosi+7]+=1;
			...
			for (j = 0; j < veh_sum_right; j++) {
				...
				if (car_right[j].x>= LA-200) {

					int intPosi=(int)car_right[j].x;

					RLdensity[intPosi]+=1;
					RLdensity[intPosi+1]+=1;
					RLdensity[intPosi+2]+=1;
					RLdensity[intPosi+3]+=1;
					RLdensity[intPosi+4]+=1;
					RLdensity[intPosi+5]+=1;
					RLdensity[intPosi+6]+=1;
					RLdensity[intPosi+7]+=1;
	...
	for(i=LA-200; i<road_long_right; i++) {

		fprintf(fp1, "%d ", i);


		fprintf(fp1, "%.3f ", (double)CLdensity[i]/count_timestep);



		fprintf(fp1, "%.3f \n", (double)RLdensity[i]/count_timestep);
