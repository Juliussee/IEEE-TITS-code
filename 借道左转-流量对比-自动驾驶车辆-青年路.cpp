...
#define total_timestep 110000
#define count_timestep 100000
...
int main() {
	...
	FILE* fp;
	if ((fp = fopen("借道左转 流量对比 自动驾驶车辆-青年路 qin=0.2.txt", "a+")) == NULL) {//这里修改5  6  10  20
	...
	for(r_pro=0; r_pro<1.0001; r_pro+=0.01) {
		printf("渗透率：%f 正在计算。。。\n",r_pro);
		...
		//不同预信号灯信号配时下的速度、密度、车头时距以及流量。

		printf("换道车辆数：%d 左车道流量：%.3f 右车道流量：%.3f \n\n",LB_change_num,(double)coil_veh_sum_left / (count_timestep*sample),(double)coil_veh_sum_right / (count_timestep*sample));

		fprintf(fp, "%.3f ", r_pro);

		fprintf(fp, "%f ", (double)coil_veh_sum_left /(count_timestep*sample)); //表示流量

		fprintf(fp, "%f ", (double)coil_veh_sum_right /(count_timestep*sample)); //表示流量

		fprintf(fp, "%f ", (double)coil_velosity_sum_left / coil_veh_sum_left); //表示速度
		
		fprintf(fp, "%f \n", (double)coil_velosity_sum_right / coil_veh_sum_right); //表示速度
	}
