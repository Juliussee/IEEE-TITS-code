...
int velocitySum(CARL* car_left, CARR* car_right) {
	...
	if(light_time==0) {
		...
		printf("time:%d Tenter:%d Tclear:%d\n",time_step,Tenter,Tclear);
	...
	for (i = 0; i <= LABlast;) {//这里sum_right的值应该改变
		if (car_right[i].isinsert == 1) {
			printf("当前车辆换道，time:%d Tenter:%d Tclear:%d 换道数：%d\n",time_step,Tenter,Tclear,LB_change_num);
...
int main() {
	...
	FILE* fp;
	if ((fp = fopen("借道左转 延误对比 汉阳大道-早高峰.txt", "a+")) == NULL) {//这里修改5  6  10  20
	...
	delay_time=0;//表示总的延误时间

	period_num=0;//表示统计时间范围内的周期数
